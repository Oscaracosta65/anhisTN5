...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', 'Analyze ' . $stateName . ' ' . $gName . ' draw history. View frequency counts, hot and quiet numbers, recency data, and draw history patterns. Powered by LottoExpert.net.');
}
...
$urlSkai    = '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=TN5';
$urlAi      = '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=TN5';
$urlSkipHit = '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=TN5';
$urlMcmc    = '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=TN5';
$urlHeatmap = '/picking-winning-numbers/artificial-intelligence/tennessee-daily-jackpot-heatmap-analysis';

$urlArchive = '/lottery-archives-pick5?gId='    . rawurlencode($gId)
    . '&stateName=' . rawurlencode((string) $stateName)
    . '&gName='     . rawurlencode((string) $gName)
    . '&sTn='       . rawurlencode(strtolower((string) $stateAbrev));

$urlLowest  = '/lowest-drawn-number-analysis?gId=' . rawurlencode($gId)
    . '&stateName=' . rawurlencode((string) $stateName)
    . '&gName='     . rawurlencode((string) $gName)
    . '&sTn='       . rawurlencode(strtolower((string) $stateAbrev));

$formAction = '/all-us-lotteries/results-analysis'
    . '?st='  . rawurlencode((string) $stateAbrev)
    . '&stn=' . rawurlencode((string) $stateName)
    . '&gm='  . rawurlencode((string) $gName)
    . '#tables';

/**
 * --------------------------------------------------------------------------
 * JSON-LD Structured Data (SEO / AIO)
 * --------------------------------------------------------------------------
 */
$jsonLdPage = json_encode([
    '@context'    => 'https://schema.org',
    '@type'       => 'WebPage',
    'name'        => 'Results Intelligence — ' . (string) $stateName . ' ' . (string) $gName,
    'description' => 'Frequency, recency, and draw history analysis for ' . (string) $stateName . ' ' . (string) $gName . '. Powered by LottoExpert.net.',
...
$doc->addCustomTag('<script type="application/ld+json">' . $jsonLdPage . '</script>');
?>
<style>
:root{
  --skai-blue:#1C66FF;
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="<?php echo htmlspecialchars($urlSkai, ENT_QUOTES, 'UTF-8'); ?>">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="<?php echo htmlspecialchars($urlAi, ENT_QUOTES, 'UTF-8'); ?>">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($urlSkipHit, ENT_QUOTES, 'UTF-8'); ?>">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($urlMcmc, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($urlHeatmap, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo str_pad((string) $mainMax, 2, '0', STR_PAD_LEFT); ?>. The right panel shows the recency distribution — how many draws have elapsed since each number last appeared.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo str_pad((string) $mainMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. <strong>Recency</strong> measures the number of draws elapsed since each number last appeared — a higher value means the number has been absent longer.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Adjust the draw window to see how frequency changes over different time horizons.
        </p>
      </div>
    </div>

    <div class="skai-controls">
      <form name="fqsearch" method="post" action="<?php echo htmlspecialchars($formAction, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo str_pad((string) $mainMax, 2, '0', STR_PAD_LEFT); ?></span>
        </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($urlSkai, ENT_QUOTES, 'UTF-8'); ?>">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($urlAi, ENT_QUOTES, 'UTF-8'); ?>">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($urlSkipHit, ENT_QUOTES, 'UTF-8'); ?>">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($urlMcmc, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($urlHeatmap, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($urlArchive, ENT_QUOTES, 'UTF-8'); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($urlLowest, ENT_QUOTES, 'UTF-8'); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
